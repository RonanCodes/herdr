...
You do not have to go looking, though. The right-click menus and the sidebar menu print the
key beside every entry that has one, written the way you press it rather than the way it is
configured: an entry bound to `prefix+c` reads `ctrl+b c` under the default prefix, and reads
your own prefix once you change it. Entries you have unbound, and entries with no keyboard
equivalent at all, show nothing. This is the intended path from the mouse to the keyboard —
keep clicking, read the key while you are there, and press it instead once it has stuck.
